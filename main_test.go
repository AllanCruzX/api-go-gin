...
func CriaAlunoMock() {
	aluno := models.Aluno{Nome: "Rodrigo Tilapia", CPF: "12345678901", RG: "123456789"}
...
func TestVerificaStatusCodeDaSaudacaoComParametro(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/allan", nil)
	resposta := httptest.NewRecorder()
	r.ServeHTTP(resposta, req)
	assert.Equal(t, http.StatusOK, resposta.Code, "Deveriam ser iguais")
	mockDaResposta := `{"API diz":"E ai allan, tudo beleza?"}`
...
func TestBuscaAlunoPorIDHandler(t *testing.T) {
	...
	assert.Equal(t, "Rodrigo Tilapia", alunoMock.Nome, "Os nomes devem ser iguais")
...
func TestEditaUmAlunoHandler(t *testing.T) {
	...
	aluno := models.Aluno{Nome: "Rodrigo Tilapia", CPF: "47123456789", RG: "123456700"}
	...
	assert.Equal(t, "Rodrigo Tilapia", alunoMockAtualizado.Nome)
}
